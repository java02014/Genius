 ...
 */
package com.java02014.genius.animation;
...
public class TouchEffectAnimator {
	...
	public void setClipRadius(float radius) {
		this.mRadii = new float[] { radius, radius, radius, radius, radius,
	...
	public void onTouchEvent(final MotionEvent event) {
		...
		if (event.getActionMasked() == MotionEvent.ACTION_UP) {
			...
			case Ripple:
				...
				mEndRadius = (float) Math.sqrt((x - mDownX) * (x - mDownX)
						+ (y - mDownY) * (y - mDownY));
				break;
			case Move:
				mStartRadius = 0;
	...
	private void initTouch() {
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP)
			mRectPath.addRoundRect(
					new RectF(0, 0, mView.getWidth(), mView.getHeight()),
					mRadii, Path.Direction.CW);
		else
			mRectPath.addRoundRect(0, 0, mView.getWidth(), mView.getHeight(),
					mRadii, Path.Direction.CW);
	}
		...
		if (mFadeInAnimation == null) {
			mFadeInAnimation = new Animation() {
				...
				protected void applyTransformation(float interpolatedTime,
					...
					case Ripple:
						mBackAlpha = (int) (interpolatedTime * mEndBackAlpha);
						mRadius = mStartRadius + (mEndRadius - mStartRadius)
								* interpolatedTime;
						break;
					case Move:
						mBackAlpha = (int) (interpolatedTime * mEndBackAlpha);
						mRadius = mEndRadius * interpolatedTime;
						mPaintX = mDownX + (mCenterX - mDownX)
								* interpolatedTime;
			...
			mFadeInAnimation
					.setAnimationListener(new TouchEffectAnimatorListener() {
						@Override
			...
			mFadeOutAnimation = new Animation() {
				...
				protected void applyTransformation(float interpolatedTime,
					...
					if (mTouchEffect == TouchEffect.Press) {
						...
						mRadius = mEndRadius + (mStartRadius - mEndRadius)
								* interpolatedTime;
					} else {
						mRadius = 0;
					}
