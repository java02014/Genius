...
public final class Command {
	...
	private static String commandRun(Command command) {
		...
		while (count > 0) {
			...
			try {
				command.mResult = I_COMMAND.command(command.mId,
						command.mTimeout, command.mParameters);
				if (command.mListener != null)
			...
			synchronized (Command.class) {
				if (EXECUTORSERVICE == null) {
					...
					EXECUTORSERVICE = Executors
							.newFixedThreadPool(size > 0 ? size : 1);
				}
			}
		}
	 ...
	 * 
	 * @param params
	 *            params eg: "/system/bin/ping", "-c", "4", "-s",
	 *            "100","www.qiujuer.net"
	 */
