...
public class CommandService extends Service {
	...
	private class CommandServiceImpl extends ICommandInterface.Stub {
				...
				public void run() {
					...
					while (mTimeoutThread == this && !this.isInterrupted()) {
						if (mCommandExecutorMap != null
								&& mCommandExecutorMap.size() > 0) {
							try {
								mMapLock.lock();
								Collection<CommandExecutor> commandExecutors = mCommandExecutorMap
										.values();
		 ...
		 */
		@Override
		public String command(String id, int timeout, String params)
