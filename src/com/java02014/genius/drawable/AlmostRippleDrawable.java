 ...
 */
public class AlmostRippleDrawable extends PaintStateDrawable implements
		Animatable {
	...
	public void draw(Canvas canvas, Paint paint) {
		...
		if (scale > INACTIVE_SCALE) {
			if (bgColor != 0) {
				...
				canvas.drawCircle(bounds.centerX(), bounds.centerY(), radius,
						paint);
			}
			if (rippleColor != 0) {
	...
	public void setColorStateList(ColorStateList tintStateList) {
		...
		mFocusedColor = tintStateList.getColorForState(
				new int[] { android.R.attr.state_focused }, 0xFFFF0000);
		mPressedColor = tintStateList.getColorForState(
				new int[] { android.R.attr.state_pressed }, 0xFFFF0000);
		mDisabledColor = tintStateList.getColorForState(
				new int[] { -android.R.attr.state_enabled }, 0xFFFF0000);
	}
