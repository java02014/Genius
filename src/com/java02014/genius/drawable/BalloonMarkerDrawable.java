 ...
 * @hide
 */
public class BalloonMarkerDrawable extends PaintStateDrawable implements
		Animatable {
	...
	// value to store que current scale when starting an animation and
	// interpolate from it
	...
	public void setColorStateList(ColorStateList tintStateList) {
		...
		mStartColor = tintStateList.getColorForState(
				new int[] { android.R.attr.state_pressed }, mEndColor);
	}
	...
	private void computePath(Rect bounds) {
		...
		float currentSize = initial + ((float) totalSize - initial)
				* currentScale;

		float halfSize = currentSize / 2f;
		float inverseScale = 1f - currentScale;
		float cornerSize = halfSize * inverseScale;
		float[] corners = new float[] { halfSize, halfSize, halfSize, halfSize,
				halfSize, halfSize, cornerSize, cornerSize };
		rect.set(bounds.left, bounds.top, bounds.left + currentSize, bounds.top
				+ currentSize);
		...
		float hDiff = (bounds.bottom - currentSize - mExternalOffset)
				* inverseScale;
	...
	public void animateToNormal() {
		...
		if (mCurrentScale > 0) {
			...
			mDuration = ANIMATION_DURATION
					- (int) (ANIMATION_DURATION * durationFactor);
			mStartTime = SystemClock.uptimeMillis();
			scheduleSelf(mUpdater, mStartTime + FRAME_DURATION);
		} else {
			notifyFinishedToListener();
		...
		public void run() {
			...
			if (diff < mDuration) {
				float interpolation = mInterpolator
						.getInterpolation((float) diff / (float) mDuration);
	...
	private static int blendColors(int color1, int color2, float factor) {
		...
		float a = (Color.alpha(color1) * factor)
				+ (Color.alpha(color2) * inverseFactor);
		float r = (Color.red(color1) * factor)
				+ (Color.red(color2) * inverseFactor);
		float g = (Color.green(color1) * factor)
				+ (Color.green(color2) * inverseFactor);
		float b = (Color.blue(color1) * factor)
				+ (Color.blue(color2) * inverseFactor);
		return Color.argb((int) a, (int) r, (int) g, (int) b);
	}

	/**
	 * A listener interface to porpagate animation events This is the
	 * "poor's man" AnimatorListener for this Drawable
	 */
