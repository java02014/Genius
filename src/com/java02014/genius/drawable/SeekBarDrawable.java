 ...
 */
package com.java02014.genius.drawable;
 ...
 * <p>
 * Special {@link com.java02014.genius.drawable.SeekBarStatusDrawable}
 * implementation To draw the Track, Scrubber and Thumb circle.
 * </p>
 * <p>
 * It's special because it will stop drawing once the state is pressed/focused
 * BUT only after a small delay.
 * </p>
 * <p>
 * This special delay is meant to help avoiding frame glitches while the
 * {@link BalloonMarkerDrawable} is added to the Window
 * </p>
 * 
 * @hide
 */
public class SeekBarDrawable extends SeekBarStatusDrawable implements
		Animatable {
	...
	public SeekBarDrawable(ColorStateList trackStateList,
			ColorStateList scrubberStateList, ColorStateList thumbStateList) {
	...
	public void copyTouchBounds(Rect rect) {
		...
		rect.set(x - mTouchRadius, bounds.top, x + mTouchRadius, bounds.bottom);
	}
	...
	protected void onBoundsChange(Rect bounds) {
		mContentWidth = bounds.right - bounds.left - mTouchRadius
				- mTouchRadius;
	...
	@Override
	public void draw(Canvas canvas, Paint paint, int trackColor,
			int trackAlpha, int scrubberColor, int scrubberAlpha,
			int thumbColor, int thumbAlpha) {
		float halfTrackStroke = mTrackStroke / 2;
		float halfScrubberStroke = mScrubberStroke / 2;

		if (isRtl) {
			draw(canvas, paint, thumbColor, thumbAlpha, trackColor,
					scrubberColor, trackAlpha, scrubberAlpha, halfTrackStroke,
					halfScrubberStroke);
		} else {
			draw(canvas, paint, thumbColor, thumbAlpha, scrubberColor,
					trackColor, scrubberAlpha, trackAlpha, halfScrubberStroke,
					halfTrackStroke);
		}
	}

	private void draw(Canvas canvas, Paint paint, int thumbColor,
			int thumbAlpha, int colorLeft, int colorRight, int alphaLeft,
			int alphaRight, float halfLeft, float halfRight) {
		Rect bounds = getBounds();
		int thumbX = mPoint.x;
		int thumbY = mPoint.y;
		...
		if (halfLeft > 0) {
			...
			canvas.drawRect(startLeft, thumbY - halfLeft, thumbX, thumbY
					+ halfLeft, paint);
		}
		...
		if (halfRight > 0) {
			...
			canvas.drawRect(thumbX, thumbY - halfRight, startRight, thumbY
					+ halfRight, paint);
		}
