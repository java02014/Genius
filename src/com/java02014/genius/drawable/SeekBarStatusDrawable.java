 ...
 * For SeekBarDrawable {@link com.java02014.genius.drawable.SeekBarDrawable}
 * 
 * @Hide
 */
public abstract class SeekBarStatusDrawable extends Drawable {
	...
	public SeekBarStatusDrawable(ColorStateList trackStateList,
	...
	public boolean isStateful() {
		return (mTrackStateList.isStateful() && mScrubberStateList.isStateful() && mThumbStateList
				.isStateful()) || super.isStateful();
	}
	...
	public void draw(Canvas canvas) {
		...
		draw(canvas, mPaint, mTrackColor, trackAlpha, mScrubberColor,
				scrubberAlpha, mThumbColor, thumbAlpha);
	...
	public ColorStateList[] getColor() {
		return new ColorStateList[] { mTrackStateList, mScrubberStateList,
				mThumbStateList };
	}
	 ...
	 * 
	 * @param canvas
	 *            The current {@link android.graphics.Canvas} to draw into
	 * @param paint
	 *            The {@link android.graphics.Paint} the Paint object that
	 *            defines with the current
	 *            {@link android.content.res.ColorStateList} color
	 */
	public abstract void draw(Canvas canvas, Paint paint, int trackColor,
	...
	private boolean updateTintColor(int[] state) {
		final int trackColor = mTrackStateList.getColorForState(state,
				mTrackColor);
		final int scrubberColor = mScrubberStateList.getColorForState(state,
				mScrubberColor);
		final int thumbColor = mThumbStateList.getColorForState(state,
				mThumbColor);

		if (trackColor != mTrackColor || scrubberColor != mScrubberColor
