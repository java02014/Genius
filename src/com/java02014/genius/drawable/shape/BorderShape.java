 ...
 */
package com.java02014.genius.drawable.shape;
...
/**
 * This class is draw line on border Line is support dash line This class is
 * extend shape Use to ShapeDrawable class
 */
public class BorderShape extends Shape {
	...
	public BorderShape(RectF border, float dashWidth, float dashGap) {
		if (border.left != 0 || border.top != 0 || border.right != 0
			...
			if (dashWidth > 0 && dashGap > 0) {
				mPathEffect = new DashPathEffect(new float[] { dashWidth,
						dashGap }, 0);
				mPath = new Path();
			}
		}
	...
	public void draw(Canvas canvas, Paint paint) {
		...
		if (mPathEffect == null) {
			...
			// bottom
			if (mBorder.bottom > 0)
				canvas.drawRect(0, height - mBorder.bottom, width, height,
						paint);
			...
			if (mBorder.right > 0) {
				...
				initPath(width - mBorder.right / 2, 0, width - mBorder.right
						/ 2, height);
				canvas.drawPath(mPath, paint);
			}
			...
			if (mBorder.bottom > 0) {
				...
				initPath(0, height - mBorder.bottom / 2, width, height
						- mBorder.bottom / 2);
				canvas.drawPath(mPath, paint);
			}
		}
