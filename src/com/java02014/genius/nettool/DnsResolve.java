 ...
 */
package com.java02014.genius.nettool;
...
public class DnsResolve extends NetModel {
	 ...
	 *            Domain name address
	 * @param server
	 *            Specify the domain name server
	 */
	...
	private ArrayList<String> resolve(String domain, InetAddress dnsServer) {
		...
		try {
			...
			// Send
			DatagramPacket dp = new DatagramPacket(sendBuffer, pos, dnsServer,
					53);
			ds.send(dp);
		...
		for (int i = 0; i < answerCount; i++) {
			...
			if (queryType == (byte) 0x01) {
				...
				iPs.add(String.format("%d.%d.%d.%d", address[0], address[1],
						address[2], address[3]));
			}
			pos += dataLength;
		}
	...
	public String toString() {
		return "Delay:" + mDelay + " IPs:"
				+ (mIPs == null ? "[]" : mIPs.toString());
	}
}