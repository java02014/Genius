...
import com.java02014.genius.Genius;
...
public final class Log {
	...
	private static final SimpleDateFormat FORMATTER = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
	private static final SimpleDateFormat FORMATTER_SIMPLE = new SimpleDateFormat(
			"HH:mm:ss");
	 ...
	 */
	/**
	 * Set Level: NOTHING = 6 ERROR = 5 WARN = 4 INFO = 3 DEBUG = 3 VERBOSE = 1
	 * ALL = 0
	 * 
	 * @param level
	 *            Level
	...
	/**
	 * Set whether to open the log storage; open the log will be stored in the
	 * software directory
	 * 
	 * @param isOpen
	 *            Open
	 * @param fileCount
	 *            File Count,Default 10 Size
	 * @param fileSize
	 *            One File Size,Default 2Mb
	 */
	public static void setSaveLog(boolean isOpen, int fileCount, float fileSize) {
		if (Genius.getApplication() == null)
			throw new NullPointerException(
					"Application is not null.Please Genius.initialize(Application)");

		if (isOpen) {
			if (Writer == null)
				Writer = new LogWriter(fileCount, fileSize,
						LogWriter.getDefaultLogPath());
	...
	/**
	 * set open or close to monitor external storage device status; insert under
	 * open external storage device will automatically copy storage after the
	 * logs to the external devices this setting depends on whether to open the
	 * log storage
	 * 
	 * @param isCopy
	 *            Copy
	 * @param filePath
	 *            Copy of the storage folder, NULL using the default, closing to
	 *            NULL;Default: SD/Genius/Logs
	 */
	...
	/**
	 * Copy log to ExternalStorage This method depends on whether to open the
	 * log storage The address can be integrated setCopyExternalStorage() method
	 * of the parameters on filePath is Null
	 * 
	 * @param filePath
	 *            Copy of the storage folder, NULL using the default, closing to
	 *            NULL;Default: SD/Genius/Logs
	...
	private static int callLog(Log log) {
		if (IsCallLog)
			return android.util.Log.println(log.getLevel(), log.getTag(),
					log.getMsg());
		else
			return 0;
	}
