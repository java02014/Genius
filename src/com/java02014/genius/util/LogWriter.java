...
class LogWriter extends Thread {
	...
	protected void registerBroadCast(String path) {
		...
		Genius.getApplication()
				.registerReceiver(mUsbBroadCastReceiver, iFilter);
	}
	...
	protected static String getDefaultLogPath() {
		return Genius.getApplication().getApplicationContext().getFilesDir()
				.getAbsolutePath()
				+ File.separator + "Genius" + File.separator + "Logs";
	}
	...
	protected void copyLogFile(String path) {
		...
		String sdFilePath = Environment.getExternalStorageDirectory()
				.getAbsolutePath()
				+ File.separator
				+ (path == null ? mExternalStoragePath : path);
		...
		if (file.isDirectory()) {
			...
			for (File logFile : allFiles) {
				...
				mWriteLock.lock();
				Tools.copyFile(logFile, new File(sdFilePath + File.separator
						+ fileName));
				mWriteLock.unlock();
			}
		}
