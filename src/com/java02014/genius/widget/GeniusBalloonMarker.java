...
import com.java02014.genius.widget.compat.GeniusCompat;
 ...
 */
public class GeniusBalloonMarker extends ViewGroup implements
	...
	public GeniusBalloonMarker(Context context, AttributeSet attrs,
			int defStyleAttr, String maxValue) {
		...
		mBalloonMarkerDrawable = new BalloonMarkerDrawable(
				ColorStateList.valueOf(Color.TRANSPARENT), 0);
		...
		if (attrs != null) {
			...
			TypedArray a = context.obtainStyledAttributes(attrs,
					R.styleable.GeniusBalloonMarker,
					R.attr.GeniusBalloonMarkerStyle,
					R.style.DefaultBalloonMarkerStyle);
			int textAppearanceId = a.getResourceId(
					R.styleable.GeniusBalloonMarker_g_markerTextAppearance,
					R.style.DefaultBalloonMarkerTextAppearanceStyle);

			setTextAppearance(textAppearanceId);

			ColorStateList color = a
					.getColorStateList(R.styleable.GeniusBalloonMarker_g_markerBackgroundColor);
			setBackgroundColor(color);

			// Elevation for android 5+
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
				float elevation = a.getDimension(
						R.styleable.GeniusBalloonMarker_g_markerElevation,
	...
	public void resetSizes(String maxValue) {
		...
		// content changes
		int wSpec = MeasureSpec.makeMeasureSpec(displayMetrics.widthPixels,
				MeasureSpec.AT_MOST);
		int hSpec = MeasureSpec.makeMeasureSpec(displayMetrics.heightPixels,
				MeasureSpec.AT_MOST);
		mNumber.measure(wSpec, hSpec);
		mWidth = Math.max(mNumber.getMeasuredWidth(),
				mNumber.getMeasuredHeight());
		removeView(mNumber);
		addView(mNumber, new FrameLayout.LayoutParams(mWidth, mWidth,
	...
	public void animateClose() {
		mBalloonMarkerDrawable.stop();
		ViewCompat.animate(mNumber).alpha(0f).setDuration(100)
				.withEndAction(new Runnable() {
					@Override
					public void run() {
						// We use INVISIBLE instead of GONE to avoid a
						// requestLayout
						mNumber.setVisibility(View.INVISIBLE);
	...
	public void onOpeningComplete() {
		...
		if (getParent() instanceof BalloonMarkerDrawable.MarkerAnimationListener) {
			((BalloonMarkerDrawable.MarkerAnimationListener) getParent())
					.onOpeningComplete();
		}
	}
