...
import com.java02014.genius.animation.TouchEffect;
import com.java02014.genius.animation.TouchEffectAnimator;
...
import com.java02014.genius.widget.attribute.ButtonAttributes;
...
public class GeniusButton extends Button implements
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		if (attrs != null) {
			...
			TypedArray a = getContext().obtainStyledAttributes(attrs,
					R.styleable.GeniusButton, defStyle, 0);

			// Getting common attributes
			int customTheme = a.getResourceId(R.styleable.GeniusButton_g_theme,
			...
			mAttributes.setFontFamily(BaseAttributes.DEFAULT_FONT_FAMILY[a
					.getInt(R.styleable.GeniusButton_g_fontFamily, 0)]);
			mAttributes.setFontWeight(BaseAttributes.DEFAULT_FONT_WEIGHT[a
					.getInt(R.styleable.GeniusButton_g_fontWeight, 3)]);
			mAttributes.setFontExtension(a
					.getString(R.styleable.GeniusButton_g_fontExtension));
			mAttributes.setTextAppearance(a.getInt(
					R.styleable.GeniusButton_g_textAppearance,
					BaseAttributes.DEFAULT_TEXT_APPEARANCE));
			mAttributes.setBorderWidth(a.getDimensionPixelSize(
					R.styleable.GeniusButton_g_borderWidth,
					BaseAttributes.DEFAULT_BORDER_WIDTH));

			// Set init Corners Radius
			mAttributes.initCornerRadius(a,
					R.styleable.GeniusButton_g_cornerRadius,
					R.styleable.GeniusButton_g_cornerRadii_A,
					R.styleable.GeniusButton_g_cornerRadii_B,
					R.styleable.GeniusButton_g_cornerRadii_C,
					R.styleable.GeniusButton_g_cornerRadii_D);

			// Getting view specific attributes
			mAttributes.setBottom(a.getDimensionPixelSize(
					R.styleable.GeniusButton_g_blockButtonEffectHeight,
					mAttributes.getBottom()));
			setTouchEffect(a.getInt(R.styleable.GeniusButton_g_touchEffect, 3));
			setTouchEffectColor(a.getColor(
					R.styleable.GeniusButton_g_touchEffectColor, -1));
			mAttributes.setDelayClick(a.getBoolean(
					R.styleable.GeniusButton_g_delayClick,
		...
		if (!mAttributes.isHasOwnBackground()) {
			...
			if (mTouchEffectAnimator == null)
				states.addState(new int[] { android.R.attr.state_pressed,
						android.R.attr.state_enabled }, background[1]);
	...
	private Drawable[] initBackground() {
		...
		ShapeDrawable normalFront = new ShapeDrawable(new RoundRectShape(
				mAttributes.getOuterRadii(), null, null));
	...
	public boolean performClick() {
		if (mAttributes.isDelayClick() && mTouchEffectAnimator != null) {
			return !mTouchEffectAnimator.interceptClick()
					&& super.performClick();
		} else
