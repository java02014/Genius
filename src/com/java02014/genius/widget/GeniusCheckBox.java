...
import com.java02014.genius.widget.attribute.CheckBoxAttributes;
...
/**
 * This is CheckBox widget The widget extend view widget
 */
public class GeniusCheckBox extends View implements Checkable,
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		if (attrs != null) {
			...
			final TypedArray a = getContext().obtainStyledAttributes(attrs,
					R.styleable.GeniusCheckBox, defStyle, 0);

			// getting common attributes
			int customTheme = a.getResourceId(
					R.styleable.GeniusCheckBox_g_theme,
					Attributes.DEFAULT_THEME);
			mAttributes.setTheme(customTheme, getResources());

			// getting custom attributes
			mAttributes.setRingWidth(a.getDimensionPixelSize(
					R.styleable.GeniusCheckBox_g_ringWidth,
					mAttributes.getRingWidth()));
			mAttributes.setCircleRadius(a.getDimensionPixelSize(
					R.styleable.GeniusCheckBox_g_circleRadius,
					mAttributes.getCircleRadius()));
			mAttributes
					.setCustomCircleRadius(mAttributes.getCircleRadius() != Attributes.INVALID);
	...
	private void initSize() {
		...
		if (w > 0 && h > 0) {
			...
			if (mOval == null)
				mOval = new RectF(mCenterX - areRadius, mCenterY - areRadius,
						mCenterX + areRadius, mCenterY + areRadius);
			else {
				mOval.set(mCenterX - areRadius, mCenterY - areRadius, mCenterX
						+ areRadius, mCenterY + areRadius);
			}

			if (!mAttributes.isCustomCircleRadius())
				mAttributes.setCircleRadius(center - mAttributes.getRingWidth()
						* 2);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(mCenterX, mCenterY, mAttributes.getCircleRadius(),
				mCirclePaint);
		...
		if (mOval != null) {
			...
			canvas.drawArc(mOval, 225, mCurProperty.mSweepAngle, false,
					mRingPaint);
		}
	}
	...
	private void animateCheckedState(boolean newCheckedState) {
		...
		if (mAnimator == null) {
			mAnimator = ObjectAnimator.ofObject(this, ANIM_VALUE,
					new AnimatorEvaluator(mCurProperty), property);
	...
	private final static class AnimatorEvaluator implements
			TypeEvaluator<AnimatorProperty> {
		...
		public AnimatorProperty evaluate(float fraction,
				AnimatorProperty startValue, AnimatorProperty endValue) {
			// Values
			mProperty.mSweepAngle = (int) (startValue.mSweepAngle + (endValue.mSweepAngle - startValue.mSweepAngle)
					* fraction);
	...
	private final static Property<GeniusCheckBox, AnimatorProperty> ANIM_VALUE = new Property<GeniusCheckBox, AnimatorProperty>(
			AnimatorProperty.class, "animValue") {
		@Override
