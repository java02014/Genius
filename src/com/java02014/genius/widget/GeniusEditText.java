 ...
 */
package com.java02014.genius.widget;

import com.java02014.genius.GeniusUI;
import com.java02014.genius.drawable.shape.BorderShape;
import com.java02014.genius.sample.R;
import com.java02014.genius.widget.attribute.Attributes;
import com.java02014.genius.widget.attribute.BaseAttributes;
import com.java02014.genius.widget.attribute.EditTextAttributes;
 ...
 */
public class GeniusEditText extends EditText implements
	...
	private void init(AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			TypedArray a = getContext().obtainStyledAttributes(attrs,
					R.styleable.GeniusEditText);

			// getting common mAttributes
			int customTheme = a.getResourceId(
					R.styleable.GeniusEditText_g_theme,
					Attributes.DEFAULT_THEME);
			mAttributes.setTheme(customTheme, getResources());

			mAttributes.setFontFamily(BaseAttributes.DEFAULT_FONT_FAMILY[a
					.getInt(R.styleable.GeniusEditText_g_fontFamily, 0)]);
			mAttributes.setFontWeight(BaseAttributes.DEFAULT_FONT_WEIGHT[a
					.getInt(R.styleable.GeniusEditText_g_fontWeight, 3)]);
			mAttributes.setFontExtension(a
					.getString(R.styleable.GeniusEditText_g_fontExtension));

			mAttributes.setTextAppearance(a.getInt(
					R.styleable.GeniusEditText_g_textAppearance,
					BaseAttributes.DEFAULT_TEXT_APPEARANCE));
			mAttributes.setBorderWidth(a.getDimensionPixelSize(
					R.styleable.GeniusEditText_g_borderWidth,
					BaseAttributes.DEFAULT_BORDER_WIDTH));

			// Set init Corners Radius
			mAttributes.initCornerRadius(a,
					R.styleable.GeniusEditText_g_cornerRadius,
					R.styleable.GeniusEditText_g_cornerRadii_A,
					R.styleable.GeniusEditText_g_cornerRadii_B,
					R.styleable.GeniusEditText_g_cornerRadii_C,
					R.styleable.GeniusEditText_g_cornerRadii_D);

			// getting view specific mAttributes
			mAttributes.setStyle(a.getInt(
					R.styleable.GeniusEditText_g_fieldStyle,
					mAttributes.getStyle()));

			// Set HintProperty
			mAttributes
					.setTitleTextColor(a
							.getColorStateList(R.styleable.GeniusEditText_g_titleTextColor));
			mAttributes.setTitleTextSize(a.getDimensionPixelSize(
					R.styleable.GeniusEditText_g_titleTextSize,
					mAttributes.getTitleTextSize()));
			mAttributes.setTitleTextPaddingLeft(a.getDimensionPixelSize(
					R.styleable.GeniusEditText_g_titlePaddingLeft,
					getPaddingLeft()));
			mAttributes.setTitleTextPaddingTop(a.getDimensionPixelSize(
					R.styleable.GeniusEditText_g_titlePaddingTop,
					mAttributes.getTitleTextPaddingTop()));
			showTitle = a.getBoolean(R.styleable.GeniusEditText_g_showTitle,
					mCurTitleProperty == null || showTitle);
			a.recycle();
		}
		...
		if (isShow != mAttributes.isShowTitle()) {
			...
			if (isShow) {
				...
				setPadding(paddingLeft,
						paddingTop + mAttributes.getTitleTextSize(),
	...
	private void initBackground() {
		...
		case EditTextAttributes.STYLE_FILL: {
			...
			ShapeDrawable normal = new ShapeDrawable(new RoundRectShape(
					mAttributes.getOuterRadiiNull(), null, null));
			normal.getPaint().setColor(mAttributes.getColor(2));

			// Creating pressed state drawable
			ShapeDrawable pressed = new ShapeDrawable(new RoundRectShape(
					mAttributes.getOuterRadiiNull(), null, null));
			pressed.getPaint().setColor(mAttributes.getColor(1));

			// Creating disabled state drawable
			ShapeDrawable disabled = new ShapeDrawable(new RoundRectShape(
					mAttributes.getOuterRadiiNull(), null, null));
		...
		case EditTextAttributes.STYLE_BOX: {
			...
			normal.setStroke(mAttributes.getBorderWidth(),
					mAttributes.getColor(2));

			// Creating pressed state drawable
			GradientDrawable pressed = new GradientDrawable();
			pressed.setCornerRadii(mAttributes.getOuterRadiiNull());
			pressed.setColor(Color.WHITE);
			pressed.setStroke(mAttributes.getBorderWidth(),
					mAttributes.getColor(1));
			...
			disabled.setStroke(mAttributes.getBorderWidth(),
					mAttributes.getColor(3));
		...
		case EditTextAttributes.STYLE_LINE: {
			// Creating normal state drawable
			ShapeDrawable normal = new ShapeDrawable(new BorderShape(new RectF(
					0, 0, 0, mAttributes.getBorderWidth())));
			normal.getPaint().setColor(mAttributes.getColor(2));
			...
			ShapeDrawable pressed = new ShapeDrawable(
					new BorderShape(
							new RectF(
									...
									0,
									0,
									mAttributes.getBorderWidth() > 0 ? mAttributes
											.getBorderWidth()
											+ getContext()
													.getResources()
													.getDimensionPixelSize(
															R.dimen.genius_editText_lineStyle_selectBorder)
											: 0)));
			pressed.getPaint().setColor(mAttributes.getColor(1));

			// Creating disabled state drawable
			ShapeDrawable disabled = new ShapeDrawable(new BorderShape(
					new RectF(0, 0, 0, mAttributes.getBorderWidth() / 2),
					mAttributes.getBorderWidth() / 2,
					mAttributes.getBorderWidth()));
			disabled.getPaint().setColor(mAttributes.getColor(3));
			// disabled.getPaint().setAlpha(0xA0);
	...
	private void initTextColor() {
		...
		case EditTextAttributes.STYLE_FILL:
			if (mAttributes.getTextAppearance() == 1)
				...
				colors = (createColorStateList(0xEEFFFFFF, 0x2E000000));
			break;
		case EditTextAttributes.STYLE_BOX:
			if (mAttributes.getTextAppearance() == 1)
				colors = (createColorStateList(mAttributes.getColor(1),
						mAttributes.getColor(1) & 0x2E000000));
			else if (mAttributes.getTextAppearance() == 2)
				colors = (createColorStateList(mAttributes.getColor(3),
						mAttributes.getColor(3) & 0x2E000000));
			else
				colors = (createColorStateList(mAttributes.getColor(2),
						mAttributes.getColor(2) & 0x2E000000));
			break;
		case EditTextAttributes.STYLE_TRANSPARENT:
			if (mAttributes.getTextAppearance() == 1)
				colors = (createColorStateList(mAttributes.getColor(0),
						mAttributes.getColor(0) & 0x2E000000));
			else if (mAttributes.getTextAppearance() == 2)
				colors = (createColorStateList(mAttributes.getColor(2),
						mAttributes.getColor(2) & 0x2E000000));
			else
	...
	private void initHintTextColor() {
		...
		switch (mAttributes.getStyle()) {
		case EditTextAttributes.STYLE_FILL:
			colors = (createColorStateList(mAttributes.getColor(3), 0xEEFFFFFF,
					0x40000000));
			break;
		case EditTextAttributes.STYLE_BOX:
			colors = (createColorStateList(mAttributes.getColor(3),
					mAttributes.getColor(2),
					mAttributes.getColor(3) & 0x00ffffff | 0x90000000));
			break;
		default:
			colors = (createColorStateList(mAttributes.getColor(3),
					mAttributes.getColor(2),
	...
	private int getCurrentTitleTextColor() {
		if (mAttributes.getTitleTextColor() == null) {
			...
			return mAttributes.getTitleTextColor().getColorForState(
					getDrawableState(), 0);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mAttributes.isShowTitle() && mTitlePaint != null
				&& mCurTitleProperty != null && mCurTitleProperty.mAlpha != 0) {
			CharSequence buf = getHint();
			if (buf != null) {
				mTitlePaint.setTextSize(mCurTitleProperty.mTextSize);

				int color = getCurrentTitleTextColor();
				int alpha = GeniusUI.modulateAlpha(Color.alpha(color),
						mCurTitleProperty.mAlpha);
				...
				canvas.drawText(buf, 0, buf.length(),
						...
						mCurTitleProperty.mPaddingTop
								+ mCurTitleProperty.mTextSize, mTitlePaint);
			}
		}
	...
	private StateListDrawable createStateListDrawable(Drawable drawable[]) {
		...
		states.addState(new int[] { android.R.attr.state_pressed,
				android.R.attr.state_enabled }, drawable[0]);
	...
	private void animateShowTitle(boolean show) {
		...
		if (isAttachWindow) {
			if (mAnimator == null) {
				mAnimator = ObjectAnimator.ofObject(this, TITLE_PROPERTY,
						new TitleEvaluator(mCurTitleProperty), property);
	...
	private final static class TitleEvaluator implements
			TypeEvaluator<TitleProperty> {
		...
		@Override
		public TitleProperty evaluate(float fraction, TitleProperty startValue,
				TitleProperty endValue) {
			// Values
			mProperty.mPaddingLeft = (int) (startValue.mPaddingLeft + (endValue.mPaddingLeft - startValue.mPaddingLeft)
					* fraction);
			mProperty.mPaddingTop = (int) (startValue.mPaddingTop + (endValue.mPaddingTop - startValue.mPaddingTop)
					* fraction);
			mProperty.mTextSize = (int) (startValue.mTextSize + (endValue.mTextSize - startValue.mTextSize)
					* fraction);
			mProperty.mAlpha = (int) (startValue.mAlpha + (endValue.mAlpha - startValue.mAlpha)
					* fraction);
			return mProperty;
		}
	}

	private final static Property<GeniusEditText, TitleProperty> TITLE_PROPERTY = new Property<GeniusEditText, TitleProperty>(
			TitleProperty.class, "titleProperty") {
		@Override
